...
export async function navigateToFullPage(context) {
  ...
  // We only need the pageId, so omit spaceId and spaceKey
  ...
  // Build query parameters with page context (only including pageId)
  const params = new URLSearchParams();
  if (pageId) params.append('pageId', pageId);

  // Construct full Forge URL, only including pageId as a query param if present
